...
print(ori, result)  # result, ori는 별개. copy는 사본 생성이다~!
...
subjects = ["국어", "수학", "영어", "과학"]  # 매칭할게 없으면 매칭 x
scores = [80, 90, 95]

print(zip(subjects, scores))

a, b, c = zip(subjects, scores)  # strict=True -> 에러(매칭할 게 없으면)
print(a, b, c)
...
# list(x)는 x를 순회해서 데이터 하나하나씩 꺼내서 리스트에 넣음
...
result = [sum(scores) / len(scores) for scores in zip(std1, std2, std3)]
